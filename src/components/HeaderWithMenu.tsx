import React, { useState } from "react";

// Hamburger Menu Icon
const MenuIcon: React.FC = () => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    className="h-7 w-7"
    fill="none"
    viewBox="0 0 24 24"
    stroke="currentColor"
    strokeWidth={1.5}
  >
    <path
      strokeLinecap="round"
      strokeLinejoin="round"
      d="M4 6h16M4 12h16M4 18h16"
    />
  </svg>
);

// Close (X) Icon
const CloseIcon: React.FC = () => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    className="h-7 w-7"
    fill="none"
    viewBox="0 0 24 24"
    stroke="currentColor"
    strokeWidth={1.5}
  >
    <path
      strokeLinecap="round"
      strokeLinejoin="round"
      d="M6 18L18 6M6 6l12 12"
    />
  </svg>
);

// Instagram Icon (replace with your SVG if needed)
const InstagramIcon: React.FC = () => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    fill="currentColor"
    viewBox="0 0 24 24"
    className="h-7 w-7"
  >
    <path d="M7.75 2h8.5A5.75 5.75 0 0 1 22 7.75v8.5A5.75 5.75 0 0 1 16.25 22h-8.5A5.75 5.75 0 0 1 2 16.25v-8.5A5.75 5.75 0 0 1 7.75 2zm0 1.5A4.25 4.25 0 0 0 3.5 7.75v8.5A4.25 4.25 0 0 0 7.75 20.5h8.5A4.25 4.25 0 0 0 20.5 16.25v-8.5A4.25 4.25 0 0 0 16.25 3.5zm4.25 3.25a5.25 5.25 0 1 1 0 10.5 5.25 5.25 0 0 1 0-10.5zm0 1.5a3.75 3.75 0 1 0 0 7.5 3.75 3.75 0 0 0 0-7.5zm5.13.62a1.13 1.13 0 1 1-2.25 0 1.13 1.13 0 0 1 2.25 0z" />
  </svg>
);

export default function HeaderWithMenu({ title }: { title: string }) {
  const [isOpen, setIsOpen] = useState(false);

  const handleLinkClick = () => setIsOpen(false);

  return (
    <>
      {/* Header Bar */}
      <header className="w-full bg-gray-900 flex items-center justify-between px-6 py-4 relative z-50">
        {/* Menu Button */}
        <button
          className="text-white focus:outline-none"
          aria-label="Open menu"
          onClick={() => setIsOpen(true)}
        >
          <MenuIcon />
        </button>
        {/* Centered Title */}
        <h1 className="text-white text-2xl font-thin absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 m-0">
          {title}
        </h1>
        {/* Name on the right */}
        <span className="text-white text-lg font-thin">
          Natalia Chapa Photography
        </span>
      </header>

      {/* Side Menu Overlay */}
      <div
        className={`fixed inset-0 bg-black bg-opacity-30 z-40 transition-opacity duration-300 ease-in-out ${
          isOpen ? "opacity-100" : "opacity-0 pointer-events-none"
        }`}
        onClick={() => setIsOpen(false)}
        aria-hidden="true"
      />

      {/* Side Menu Drawer */}
      <nav
        className={`fixed top-0 left-0 h-full w-full max-w-xs bg-white shadow-2xl z-50 transform transition-transform duration-300 ease-in-out ${
          isOpen ? "translate-x-0" : "-translate-x-full"
        }`}
      >
        <div className="h-full flex flex-col items-center justify-center">
          <button
            className="absolute top-6 right-6 text-gray-700"
            aria-label="Close menu"
            onClick={() => setIsOpen(false)}
          >
            <CloseIcon />
          </button>
          <ul className="space-y-12 text-center">
            <li>
              <a
                href="/"
                onClick={handleLinkClick}
                className="text-3xl font-light text-gray-700 hover:text-black transition-all duration-300 transform hover:scale-110 inline-block"
              >
                Home
              </a>
            </li>
            <li>
              <a
                href="/people"
                onClick={handleLinkClick}
                className="text-3xl font-light text-gray-700 hover:text-black transition-all duration-300 transform hover:scale-110 inline-block"
              >
                People
              </a>
            </li>
            <li>
              <a
                href="/nature"
                onClick={handleLinkClick}
                className="text-3xl font-light text-gray-700 hover:text-black transition-all duration-300 transform hover:scale-110 inline-block"
              >
                Nature/Landscape
              </a>
            </li>
            <li>
              <a
                href="/animals"
                onClick={handleLinkClick}
                className="text-3xl font-light text-gray-700 hover:text-black transition-all duration-300 transform hover:scale-110 inline-block"
              >
                Animals
              </a>
            </li>
            <li>
              <a
                href="/lifestyle"
                onClick={handleLinkClick}
                className="text-3xl font-light text-gray-700 hover:text-black transition-all duration-300 transform hover:scale-110 inline-block"
              >
                Lifestyle
              </a>
            </li>
            <li className="flex justify-center">
              <a
                href="https://instagram.com/throughthequietlens_photo?"
                target="_blank"
                rel="noopener noreferrer"
                className="text-gray-500 hover:text-gray-900 transition-colors"
                aria-label="Instagram page"
              >
                <InstagramIcon />
              </a>
            </li>
          </ul>
        </div>
      </nav>
    </>
  );
}
